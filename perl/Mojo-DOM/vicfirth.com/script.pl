...
foreach my $p_data ( $dom->find('script.cue-playlist-data')->each ) {
    ...
    foreach my $item ( @{ $hash->{tracks} || {} } ) {
        if ( $item->{title} =~ m/Bronze/ig ){
            my $mp3_path = sprintf($tt, $dir, $data_name_pref, 'bronze', 'mp3');
            getstore($item->{mp3}, $mp3_path);
        } elsif ( $item->{title} =~ m/Silver/ig ){
            my $mp3_path = sprintf($tt, $dir, $data_name_pref, 'silver', 'mp3');
            getstore($item->{mp3}, $mp3_path);
        } elsif ( $item->{artist} =~ m/Bronze/ig ){
            my $mp3_path = sprintf($tt, $dir, $data_name_pref, 'app-bronze', 'mp3');
            getstore($item->{mp3}, $mp3_path);
        } elsif ( $item->{artist} =~ m/Silver/ig ){
            my $mp3_path = sprintf($tt, $dir, $data_name_pref, 'app-silver', 'mp3');
            getstore($item->{mp3}, $mp3_path);
        }
    };
};
